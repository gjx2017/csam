...
class BAMBlock(nn.Module):
    ...
    def __init__(self, channel=1024,reduction=16,dia_val=2):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # out = (1 + weight) * x
        #sigmoid后串行，通道留下，whatz
        # ca_out = self.ca(x)
        # weight1 = self.sigmoid(ca_out)
        # ca_out=x* weight1
        # sa_out=self.sa(ca_out)
        # weight = self.sigmoid(sa_out)
        # sa_out=ca_out*weight
        # out = x + sa_out
        # out = x +ca_out
        #并行0.9812
        sa_out = self.sa(x)
...
if __name__ == '__main__':
    ...
    bam = BAMBlock(channel=1024,reduction=8,dia_val=2)
    output=bam(input)
    print(output.shape)
